...
//-----------------------------------------------------------------------------
//
//  Custom script for BF9182-I.htm 
//
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
//
...
//
//  Description:    Do not allow update of New Face Amt field if Rider is not
//                  renewing.  Also blank this field out if a Total Line.
//
//  Arguments:      None
//
//  Returns:        Nothing
  ...
  for (i=1; i<100; i++)
  {
    ...
    if(NewFaceAmtCtl)  // note Table rows and associated fields may be deleted if blank
    {
      ...
      if(indCtl)
      {
        ...
        switch(indicator)
        {
          case 'R': // If renewing, change field color
            NewFaceAmtCtl.style.backgroundColor='#00FA9A'; // mediumspringgreen
...
//-----------------------------------------------------------------------------
//
//  Function:       ChkNewPremAmt
//
//  Description:    Do not allow update of New Prem Modal Amt field if Rider 
//                  is not renewing.  Also blank this field out if a Total Line.
//
//  Arguments:      None
//
//  Returns:        Nothing
...
function ChkNewPremAmt()
{
  for (i=1; i<100; i++)
  {
    NewPremAmtCtl = document.getElementById("MIR-DV-CVG-MPREM-AMT-T[" + i + "]");
    if(NewPremAmtCtl)  // note Table rows and associated fields may be deleted if blank
    {
      indCtl= document.getElementById("MIR-DV-CVG-RENEW-CD-T[" + i + "]");
      if(indCtl)
      {
        ...
        switch(indicator)
        {
          case 'X': // If NOT renewing, blank out field
            NewPremAmtCtl.style.display='none';
            NewPremAmtCtl.tabindex="";
            break;
        }
      }
    }
