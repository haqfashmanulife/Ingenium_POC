...
//******************************************************************************
//*  Component:   AppPolicyDataTrad.js                                         *
//*  Description:                                                              *
//*                                                                            *
//******************************************************************************
//*  Chg#    Release  Description                                              *
//*                                                                            *
//*  016051  6.1      Apply policy data requirements.                          *
//*  PF30    PF 3.0   Manual changes for array foramt after conversion         *
...
//-----------------------------------------------------------------------------
//
//      Functions for the AppPolicyDataTrad.htm page
//
...
function onLoadCustom()
{
    ...
    showJointColumnHeaders();

    // Update the status of the add and remove buttons
    updateButtonsState();

     var controlField = document.getElementsByName("mortYesNo");

     if(controlField[1].checked)
          showMortFields(false);
...
function changeRowVisibility(index, makeVisibile)
{
     ...
     var mortDetails = document.getElementById("mortDetails");

     // working with row + 1 to avoid the Titles row, (rem mortgage table
     // has 2 title rows)
 
     var rowcvgs = cvgTable.rows.item(index+1);
     var rowterm = termDetails.rows.item(index+1);
     var rowmort = mortDetails.rows.item(index+2);

    // Special care has to be taken for the mandatory fields.
    if (makeVisibile)
    {
          rowcvgs.style.display = "";
          changeMandatoryFor(rowcvgs, "yes");
          rowterm.style.display = "";
          rowmort.style.display = "";
    }
    else
    {
          changeMandatoryFor(rowcvgs, "no");
          rowcvgs.style.display = "none";
          rowterm.style.display = "none";
          rowmort.style.display = "none";
    }
...
function getCvgTableSize()
{
     var cgvTable = document.getElementById("cvgTable");
...
function updateButtonsState()
{
    ...
    else
    {
        ...
        var lengthOpts = document.getElementById("MIR-DV-PLAN-DESC-TXT-T[2]").length;

        // noAdditionalPlansAllowed =  (1 == ctl.length) &&
        //                                                      (("*" == ctl.options[0].value) ||
        //                                                       (""  == ctl.options[0].value));
        noAdditionalPlansAllowed =  (1 == lengthOpts) &&
                (("*" == ctl.options[0].value) || (""  == ctl.options[0].value));
...
//                                      - ChangeInsuredLink-T[1] (Search link)
//                                      This will effectively prevent the user from trying to update
//                                      Primary Insured on the basic risk plan.  These values will already
//                                      have been committed to the server on arrival at this page.
//
         ...
         switch(controllingFieldValue)
         {
            case "Y":
              ...
              case "B":
                  ...
                  break
              case "N":
                  field.setAttribute("mandatory", "no");
                  originalRow = true;
                  break
              case "":
                  break
              default:
                  alert("wrong value "+controllingFieldValue+" for the MIR-ADDL-PLAN-MAND-IND-T indicator");
         }
        }

//      Deal with MIR-CVG-AD-FACE-AMT-T

     field = document.getElementById("MIR-CVG-AD-FACE-AMT-T[" + tIndex + "]");
     controllingFieldValue = document.getElementById("MIR-PLAN-PKG-ADB-CD-T[" + tIndex + "]").value;
     ...
     switch (controllingFieldValue) 
     {
          ...
          case "":
               ...
               break
          default:
               alert("wrong value "+controllingFieldValue+" for the MIR-PLAN-PKG-ADB-CD-T indicator");
     }
    
//      Deal with MIR-CVG-WP-IND-T
    

     field = document.getElementById("MIR-CVG-WP-IND-T[" + tIndex + "]");
     controllingFieldValue = document.getElementById("MIR-PLAN-PKG-WP-CD-T[" + tIndex + "]").value;
     ...
     switch (controllingFieldValue) 
     {
          ...
          case "":
               ...
               break;
          default:
               alert("wrong value "+controllingFieldValue+" for the MIR-PLAN-PKG-WP-CD-T indicator");
     }

        controllingFieldValue  = document.getElementById("MIR-DV-CVG-PLAN-JNT-IND-T[" + tIndex + "]").value;
        ...
        if ("N" == controllingFieldValue)
        {
                ...
                var jointLifeCd = document.getElementById("MIR-CVG-JNT-LIFE-CD-T[" + tIndex + "]").value;
                if ("F" == jointLifeCd || "L" == jointLifeCd)
                {
                        document.getElementById("MIR-CVG-JNT-LIFE-CD-T["    + tIndex + "]").value     = "N";
                        document.getElementById("MIR-CVG-LIVES-INSRD-CD-T[" + tIndex + "]").value     = 1;
                        document.getElementById("MIR-DV-INSRD-CLI-NM-2-T["  + tIndex + "]").setAttribute("mandatory", "no");
                        document.getElementById("MIR-DV-INSRD-CLI-NM-2-T["  + tIndex + "]").value     = "";
                        document.getElementById("MIR-INSRD-CLI-ID-2-T["     + tIndex + "]").value     = "*";
                }
        }

        // Show or hide Joint Life Type Cell, based on controlling field...
        showJointLifeTypeCell (tIndex, controllingFieldValue);
...
function deleteCoverage()
{
    ...
    if (confirm(getUserMessage(msgCvgDelConfirm)))
    {
        ...
        // Clean up any data the user might have entered
                ...
                document.getElementById("MIR-CVG-AD-FACE-AMT-T["        + numUsedRows + "]").value = "";
                ...
                document.getElementById("MIR-CVG-JNT-LIFE-CD-T["        + numUsedRows + "]").value = "";
          // second table
          document.getElementById("MIR-CVG-MAT-XPRY-DUR-T["+ numUsedRows + "]").value = "";
          document.getElementById("MIR-CVG-MAT-XPRY-AGE-T[" + numUsedRows + "]").value = "";
          document.getElementById("MIR-CVG-MAT-XPRY-DT-T[" + numUsedRows + "]").value = "";
          // third table
          document.getElementById("MIR-ANN-MTG-INT-PCT-T["+ numUsedRows + "]").value = "";
          document.getElementById("MIR-INT-CMPND-FREQ-CD-T[" + numUsedRows + "]").value = "";
          document.getElementById("MIR-CVG-MTG-REPAY-AMT-T[" + numUsedRows + "]").value = "";
          document.getElementById("MIR-MTG-REPAY-FREQ-CD-T[" + numUsedRows + "]").value = "";

                showOtherJointCells(numUsedRows);  // Refresh display of other joint-related fields
                updateButtonsState();
    }
}
...
function onChangeInsured(nameFieldName, idFieldName)
{
        ...
        var index = parseInt(selectionBoxName.substr(selectionBoxName.length - 2, 1), 10);
...
function showMortFields(show)
{
     var mortTab = document.getElementById("mortDetails");


     if (show)
     {
          mortTab.style.display = "";
     }
     else
     {
          mortTab.style.display = "none";
     }
}
//******************************************************************************
//
//     Function:       blankAge(field)
//
//     Description:     blanks out the Age field if Duration entered
//
//     Arguments:     this object
//
//     Returns:     Nothing
//******************************************************************************
function blankAge(fieldName)
{
     // Check that a value has been added to Duration


     if (fieldName.value != "")
     {
          // Get the field name
          var thisField = fieldName.name;

          // Get the table occurence
          var lastDash = thisField.lastIndexOf("[");
          var ctlIndex = thisField.substr(lastDash +1);
          
          // clear the corresponding Age field if changed

//          var beforeTerm = parseFloat(fieldName.value);
//          var afterTerm = parseFloat(document.getElementById("savedTerm" + ctlIndex).value);

//          if (beforeTerm != afterTerm)
//          {
               document.getElementById("MIR-CVG-MAT-XPRY-AGE-T["+ctlIndex).value = "";
               document.getElementById("MIR-CVG-MAT-XPRY-DT-T["+ctlIndex).value = "";
//          }
     }
}
//******************************************************************************
//
//     Function:       blankDur(field)
//
//     Description:     blanks out the Duration field if term entered
//
//     Arguments:     this object
//
//     Returns:     Nothing
//******************************************************************************
function blankDur(fieldName)
{
     // Check that a value has been added to Age
     ...
     if (fieldName.value != "") 
     {
          ...
          // clear the value in the corresponding Term field if Age changed

//          var beforeAge = parseFloat(fieldName.value);
//          var afterAge = parseFloat(document.getElementById("savedAge" + ctlIndex).value);

//          if (beforeAge != afterAge)
//          {
               document.getElementById("MIR-CVG-MAT-XPRY-DUR-T["+ctlIndex).value = "";
               document.getElementById("MIR-CVG-MAT-XPRY-DT-T["+ctlIndex).value = "";
//          }
     }
}
//******************************************************************************
//
//     Function:       blankAgeDur(fieldName)
//
//     Description:     blanks out the Age and Duration field if date entered
//
//     Arguments:     this object
//
//     Returns:     Nothing
...
function blankAgeDur(fieldName)
{
     // Check that a value has been added to date
     ...
     if (fieldName.value != "")
     {
          ...
          // clear the value in the corresponding Dur & Age field
//          if (fieldName.value != document.getElementById("savedDate" + ctlIndex).value)
//          {
               document.getElementById("MIR-CVG-MAT-XPRY-DUR-T["+ctlIndex).value = "";
               document.getElementById("MIR-CVG-MAT-XPRY-AGE-T["+ctlIndex).value = "";
//          }
...
function onChangeJointLifeType()
{
        ...
        var index              = parseInt(selectionBox.name.substr(selectionBox.name.length-2, 1), 10);

        if (("F" == selectionBoxChoice.value) ||
                ("L" == selectionBoxChoice.value))
        {
                showOtherJointCells(index, "Y");

                document.getElementById("MIR-CVG-LIVES-INSRD-CD-T[" + index + "]").value= 2;
                document.getElementById("MIR-DV-INSRD-CLI-NM-2-T["  + index + "]").setAttribute("mandatory", "yes");
        }
        else if ((""  == selectionBoxChoice.value) ||
                         ("N" == selectionBoxChoice.value))
        {
                showOtherJointCells(index, "N");

                document.getElementById("MIR-CVG-LIVES-INSRD-CD-T[" + index + "]").value     = 1;
                document.getElementById("MIR-DV-INSRD-CLI-NM-2-T["  + index + "]").setAttribute("mandatory", "no");
                document.getElementById("MIR-DV-INSRD-CLI-NM-2-T["  + index + "]").value     = "";
                document.getElementById("MIR-INSRD-CLI-ID-2-T["     + index + "]").value     = "*";
        }
...
function onSubmitCustom()
{
                ...
                ("ACTION_NEXT"     == document.getElementById("action").value))
        {
                ...
                for (var rowIndex = 1; rowIndex <= numUsedRows; rowIndex++)
                {
...
//
//-----------------------------------------------------------------------------
function showJointLifeTypeCell (index, makeVisible)
{
        var cell = document.getElementById("cvgTable").rows[index].cells[colJointLifeType];

        if ("N" == makeVisible)
        {
                cell.style.display = "none";
        }
        else if ("Y" == makeVisible)
        {
                cell.style.display = "";
        }
...
function thereAreVisibleLorFChoices()
{
    var numRows = getCvgTableSize();
        ...
        for (var rowIndex = 1; rowIndex <= numRows; rowIndex++)
        {
                row      = table.rows[rowIndex];
                cell     = row.cells[colJointLifeType];
                ctl      = document.getElementById("MIR-CVG-JNT-LIFE-CD-T[" + rowIndex + "]");
                ctlValue = ctl.options[ctl.selectedIndex].value;

            if ((row.style.display  != "none") &&
                        (cell.style.display != "none") &&
                        ("L" == ctlValue || "F" == ctlValue))
            {
                return true;
            }
        }
