...
//******************************************************************************
//*  Component:   AppApplicationDataJPVUL.js                                   *
//*  Description:                                                              *
//*                                                                            *
//******************************************************************************
...
//-----------------------------------------------------------------------------
//
//	Functions for the AppApplicationDataJPVUL.htm page
//
//-----------------------------------------------------------------------------
...
function onChangeRelationship()
{
	if ("SAME" == getSelectionBoxValue("MIR-POL-CLI-INSRD-CD-T[1]"))
	{
		...
		//
		// This field may already be stored in the server from having selected
		// another person, so if we're going back to the same person, ensure
		// the server knows to blank this field out by sending an "*" rather than
		// just a space.
		//
		if ((null == document.getElementById("PrimaryInsuredADDR-TYP-CD-T[1]").value)             ||
			("" == cleanString(document.getElementById("PrimaryInsuredADDR-TYP-CD-T[1]").value)))
		{
			document.getElementById("MIR-CLI-ADDR-TYP-CD-T[1]").value   = "*";
		}
		else
		{
