...
#*  MM0114  BMB      Go back to the input screen when CANCEL is pressed        *
#*  M00037  CLB      Add new face amount field to screen                       *
#*  PF 2.0  PF22     PathFinder 2.0 release changes to array handling          *
#*  MFPLI5  CLB      Change for multiple switches of Life riders.              *
#*  BP0094  CLB      Enable message display on return to Input step from 9187. *
#*                   Clear it out if cancellation was requested.               *
#*  M142U2  CTS      RIDER SWITCH ENHANCEMENTS                                 *
#*  Q16009  CTS      M142U2 -  System navigates to the FTRQ Activity Create    *
#*                   Screen2 even the CSV Refund = 0 in the Rider Switch       *
...
PROCESS BF9187Switches
{
    Title = STRINGTABLE.IDS_TITLE_BF9187Switches;
    TitleBar = "TitleBar";
    TitleBarSize = "70";
...
#* MP185H Changes Begin
    MIR-DV-2-CVG-STBL-3-CD     = MIR-CVG-STBL-3-CD-T [index];
#* MP185H Changes End 
    ...
    STEP Output
    {
        ...
        "ButtonBarSwitchConfirm" -> ButtonBar;
...
# M142U2 CHANGES STARTS
    	...
    	temp-MIR-DISP-CSV-RFND-AMT = MIR-DISP-CSV-RFND-AMT; 
#Q16140 CHANGES STARTS
       temp-MIR-FTRQ-REC-IND = MIR-FTRQ-REC-IND;
#Q16140 CHANGES ENDS	
# M142U2 CHANGES ENDS
    ...
    # If the user has pressed the cancel button in the pop-up confirm box, go back to the input screen
    ...
    # If action is none of the above, then it is assumed to be Confirm.
    # This is where the new riders are added.

    STEP AddCoverages-P
    {
        USES P-STEP "BF9187-P";
    }
 ...
 # M142U2 CHANGES STARTS
#Q16009 changes starts
#    IF temp-MIR-DISP-CSV-RFND-AMT > 0
#Q16140 CHANGES STARTS
#    IF NUMBER(temp-MIR-DISP-CSV-RFND-AMT) > 0
    IF temp-MIR-FTRQ-REC-IND =="Y"
#Q16140 CHANGES ENDS
#Q16009 changes ends
    {     
        STEP CALLCR
        {
               ...
               MIR-EFF-DT -> MIR-PBRQ-EFF-DT;
               "CV" -> MIR-DV-POL-FUT-ACTV-TYP-CD;
               temp-MIR-DISP-CSV-RFND-AMT -> MIR-PMT-RQST-AMT;
        }
